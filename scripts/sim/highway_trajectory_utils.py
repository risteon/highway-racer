...
def run_highway_trajectory(
    ...
    if analysis_mode == "debug":
        print(f"{'Step':<4} | {'Action':<15} | {'Env Rew':<8} | {'Train Rew':<9} | {'Speed':<6} | {'Lane':<4} | {'Components'}")
        print("-" * 100)
    ...
    for step in range(max_steps):
            ...
            if img is not None:
                ...
                if len(present_vehicles) > 0:
                    ...
                    for i, item in enumerate(info_items):
                        ...
                        elif "Margin:" in item and "lateral" in debug_info and "lane_width" in debug_info:
                            # Red if outside lane boundaries, green if inside
                            if abs(debug_info["lateral"]) > debug_info["lane_width"] / 2:
                                text_color = (255, 0, 0)
                            else:
                                text_color = (0, 255, 0)
        ...
        training_reward, reward_components = calculate_training_reward(
            env, env_reward, info, safety_bonus_coeff,
        ...
        if len(present_vehicles) > 0:
            ...
            current_lane = int((ego_y + 2*lane_width) / lane_width)  # Rough lane estimation
        ...
        if analysis_mode == "debug":
            ...
            print(f"{step:<4} | {action_str:<15} | {env_reward:<8.3f} | {training_reward:<9.3f} | {ego_speed:<6.1f} | {current_lane:<4} | {components_str}")
    ...
    if render_video and images:
        if video_output_path is None:
            ...
            video_output_path = f"{video_output_dir}/highway_trajectory_{analysis_mode}.mp4"
        else:
