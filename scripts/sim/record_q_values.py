...
flags.DEFINE_string(
    "output_dir", "./q_value_analysis", "Path to the output directory"
)
...
flags.DEFINE_integer("num_vehicles", 15, "Number of vehicles in environment")
...
def run_episode_with_q_recording(
    agent, env, max_steps=1000, render_frames=True
):
    """
    ...
    for step in range(max_steps):
        ...
        if hasattr(agent, 'sample_actions'):
            ...
            dist = agent.actor.apply_fn(
                {"params": agent.actor.params}, obs[None, :]
            )
...
def save_episode_data(episode_data, output_dir, episode_idx):
    """
...
def main(_):
    ...
    has_distributional_critic = hasattr(agent, 'critic') and hasattr(agent.critic, 'apply_fn')
    ...
    for episode in trange(FLAGS.num_episodes, desc="Recording episodes"):
        ...
        # Save episode data
        save_episode_data(episode_data, auto_output_dir, episode)
    ...
    aggregate_data = {
        ...
        "mean_return": float(np.mean([ep["episode_return"] for ep in all_episodes_data])),
        "std_return": float(np.std([ep["episode_return"] for ep in all_episodes_data])),
        "mean_length": float(np.mean([ep["episode_length"] for ep in all_episodes_data])),
    }
    ...
    print(f"Mean return: {aggregate_data['mean_return']:.3f} ± {aggregate_data['std_return']:.3f}")
