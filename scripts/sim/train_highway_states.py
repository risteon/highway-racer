...
flags.DEFINE_string(
    "env_name", "highway-v0", "Highway environment name."
)
...
def run_trajectory(agent, env, max_steps=1000, video: bool = False, output_range: Tuple[float, float] = None):
    obs, _ = env.reset()
...
def evaluate(agent, eval_env, num_episodes: int, output_range: Tuple[float, float] = None):
    episode_returns = []
    episode_lengths = []
    episode_videos = []

    for i in range(num_episodes):
        images, episode_return, episode_length = run_trajectory(agent, eval_env, video=i<4, output_range=output_range)
...
def main(_):
    ...
    replay_buffer = ReplayBuffer(
        env.observation_space, env.action_space, replay_buffer_size, extra_fields=["safety"]
    )
    ...
    wandb.init(
        ...
        notes=FLAGS.comment, 
        group=f"{wandb_group_name}-highway" if FLAGS.group_name_suffix is None else f"{wandb_group_name}-{FLAGS.group_name_suffix}-highway"
    )
    config_for_wandb = {
        ...
        if i >= FLAGS.start_training:
            ...
            if FLAGS.expert_replay_buffer:
                ...
                for key in batch.keys():
                    batch[key] = np.concatenate([batch[key][:FLAGS.batch_size//2], 
                                               expert_batch[key][:FLAGS.batch_size//2]], axis=0)
